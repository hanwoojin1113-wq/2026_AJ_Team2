DROP TABLE IF EXISTS user_badge_selected;
DROP TABLE IF EXISTS user_badge_earned;
DROP TABLE IF EXISTS battle_share;
DROP TABLE IF EXISTS battle_comment_like;
DROP TABLE IF EXISTS battle_comment;
DROP TABLE IF EXISTS battle_vote;
DROP TABLE IF EXISTS movie_battle;
DROP TABLE IF EXISTS user_follow;
DROP TABLE IF EXISTS user_movie_collection;
DROP TABLE IF EXISTS user_movie_life;
DROP TABLE IF EXISTS user_movie_watched;
DROP TABLE IF EXISTS user_movie_store;
DROP TABLE IF EXISTS user_movie_like;
DROP TABLE IF EXISTS user_recommendation_result;
DROP TABLE IF EXISTS user_preference_profile;
DROP TABLE IF EXISTS user_recommendation_refresh_state;
DROP TABLE IF EXISTS movie_audit;
DROP TABLE IF EXISTS movie_company;
DROP TABLE IF EXISTS movie_provider;
DROP TABLE IF EXISTS movie_keyword;
DROP TABLE IF EXISTS movie_tag;
DROP TABLE IF EXISTS tmdb_trending_chart;
DROP TABLE IF EXISTS batch_job_run;
DROP TABLE IF EXISTS kobis_movie_raw;
DROP TABLE IF EXISTS tmdb_movie_raw;
DROP TABLE IF EXISTS movie_co_occurrence;
DROP TABLE IF EXISTS movie_actor;
DROP TABLE IF EXISTS movie_director;
DROP TABLE IF EXISTS movie_genre;
DROP TABLE IF EXISTS movie_source;
DROP TABLE IF EXISTS movie_throw;
DROP TABLE IF EXISTS movie;
DROP TABLE IF EXISTS "USER";
DROP TABLE IF EXISTS audit;
DROP TABLE IF EXISTS watch_grade;
DROP TABLE IF EXISTS provider;
DROP TABLE IF EXISTS keyword;
DROP TABLE IF EXISTS company;
DROP TABLE IF EXISTS person;
DROP TABLE IF EXISTS tag;
DROP TABLE IF EXISTS genre;
DROP TABLE IF EXISTS nation;
DROP TABLE IF EXISTS production_status;
DROP TABLE IF EXISTS movie_type;

CREATE TABLE movie_type (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE production_status (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE nation (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE genre (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE person (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(200) NOT NULL UNIQUE
);

CREATE TABLE company (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(255) NOT NULL UNIQUE
);

CREATE TABLE provider (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tmdb_provider_id BIGINT NOT NULL UNIQUE,
    provider_name VARCHAR(255) NOT NULL,
    logo_path VARCHAR(500),
    display_priority INT
);

CREATE TABLE keyword (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tmdb_keyword_id BIGINT,
    name VARCHAR(255) NOT NULL UNIQUE
);

CREATE TABLE tag (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tag_name VARCHAR(100) NOT NULL,
    tag_type VARCHAR(20) NOT NULL,
    CONSTRAINT uk_tag_name_type UNIQUE (tag_name, tag_type)
);

CREATE TABLE watch_grade (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE audit (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    audit_no VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE "USER" (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    login_id VARCHAR(100) NOT NULL UNIQUE,
    login_pw VARCHAR(100) NOT NULL,
    nickname VARCHAR(100) NOT NULL,
    gender VARCHAR(10) NOT NULL,
    age INT NOT NULL,
    profile_image_url VARCHAR(500),
    CONSTRAINT ck_user_gender CHECK (gender IN ('MALE', 'FEMALE'))
);

CREATE TABLE movie (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ranking INT,
    movie_cd VARCHAR(20) NOT NULL UNIQUE,
    movie_name VARCHAR(255) NOT NULL,
    movie_name_en VARCHAR(255),
    movie_name_original VARCHAR(255),
    box_office_open_date DATE,
    movie_info_open_date DATE,
    production_year INT,
    show_time INT,
    movie_type_id BIGINT,
    production_status_id BIGINT,
    nation_id BIGINT,
    poster_image_url VARCHAR(500),
    box_office_sales_acc BIGINT,
    box_office_audi_acc BIGINT,
    box_office_scrn_cnt INT,
    box_office_show_cnt INT,
    title VARCHAR(255),
    original_title VARCHAR(255),
    overview CLOB,
    release_date DATE,
    runtime INT,
    budget BIGINT,
    revenue BIGINT,
    tmdb_status VARCHAR(100),
    tagline VARCHAR(500),
    poster_path VARCHAR(500),
    backdrop_path VARCHAR(500),
    vote_average DOUBLE PRECISION,
    vote_count INT,
    popularity DOUBLE PRECISION,
    adult BOOLEAN,
    original_language VARCHAR(20),
    homepage VARCHAR(500),
    imdb_id VARCHAR(50),
    created_at TIMESTAMP,
    updated_at TIMESTAMP,
    CONSTRAINT fk_movie_type FOREIGN KEY (movie_type_id) REFERENCES movie_type(id),
    CONSTRAINT fk_movie_status FOREIGN KEY (production_status_id) REFERENCES production_status(id),
    CONSTRAINT fk_movie_nation FOREIGN KEY (nation_id) REFERENCES nation(id)
);

CREATE TABLE movie_battle (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    creator_user_id BIGINT,
    movie_a_id BIGINT NOT NULL,
    movie_b_id BIGINT,
    battle_type VARCHAR(20) NOT NULL DEFAULT 'USER',
    battle_label VARCHAR(200),
    option_a_label VARCHAR(100),
    option_b_label VARCHAR(100),
    option_a_image_url VARCHAR(500),
    option_b_image_url VARCHAR(500),
    vote_a INT NOT NULL DEFAULT 0,
    vote_b INT NOT NULL DEFAULT 0,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_battle_creator FOREIGN KEY (creator_user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_battle_a FOREIGN KEY (movie_a_id) REFERENCES movie(id),
    CONSTRAINT fk_battle_b FOREIGN KEY (movie_b_id) REFERENCES movie(id)
);

CREATE TABLE battle_vote (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    battle_id BIGINT NOT NULL,
    user_id BIGINT NOT NULL,
    voted_movie_id BIGINT,
    voted_option VARCHAR(1),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_bvote_battle FOREIGN KEY (battle_id) REFERENCES movie_battle(id),
    CONSTRAINT fk_bvote_user FOREIGN KEY (user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_bvote_movie FOREIGN KEY (voted_movie_id) REFERENCES movie(id),
    CONSTRAINT uq_battle_user UNIQUE (battle_id, user_id)
);

CREATE TABLE battle_comment (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    battle_id BIGINT NOT NULL,
    user_id BIGINT NOT NULL,
    content VARCHAR(1000) NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_bcomment_battle FOREIGN KEY (battle_id) REFERENCES movie_battle(id),
    CONSTRAINT fk_bcomment_user FOREIGN KEY (user_id) REFERENCES "USER"(id)
);

CREATE INDEX idx_battle_comment_battle_created
ON battle_comment (battle_id, created_at);

CREATE TABLE battle_comment_like (
    comment_id BIGINT NOT NULL,
    user_id BIGINT NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (comment_id, user_id),
    CONSTRAINT fk_bcomment_like_comment FOREIGN KEY (comment_id) REFERENCES battle_comment(id),
    CONSTRAINT fk_bcomment_like_user FOREIGN KEY (user_id) REFERENCES "USER"(id)
);

CREATE TABLE battle_share (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    battle_id BIGINT NOT NULL,
    sender_user_id BIGINT NOT NULL,
    receiver_user_id BIGINT NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_bshare_battle FOREIGN KEY (battle_id) REFERENCES movie_battle(id),
    CONSTRAINT fk_bshare_sender FOREIGN KEY (sender_user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_bshare_receiver FOREIGN KEY (receiver_user_id) REFERENCES "USER"(id)
);

CREATE INDEX idx_battle_share_receiver ON battle_share (receiver_user_id, created_at);

CREATE TABLE movie_source (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    movie_id BIGINT NOT NULL,
    source_type VARCHAR(20) NOT NULL,
    source_key VARCHAR(100) NOT NULL,
    created_at TIMESTAMP NOT NULL,
    updated_at TIMESTAMP NOT NULL,
    CONSTRAINT fk_movie_source_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT uk_movie_source_type_key UNIQUE (source_type, source_key),
    CONSTRAINT uk_movie_source_movie_type UNIQUE (movie_id, source_type)
);

CREATE TABLE movie_genre (
    movie_id BIGINT NOT NULL,
    genre_id BIGINT NOT NULL,
    display_order INT NOT NULL,
    PRIMARY KEY (movie_id, genre_id),
    CONSTRAINT fk_movie_genre_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT fk_movie_genre_genre FOREIGN KEY (genre_id) REFERENCES genre(id)
);

CREATE TABLE movie_director (
    movie_id BIGINT NOT NULL,
    person_id BIGINT NOT NULL,
    display_order INT NOT NULL,
    PRIMARY KEY (movie_id, person_id),
    CONSTRAINT fk_movie_director_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT fk_movie_director_person FOREIGN KEY (person_id) REFERENCES person(id)
);

CREATE TABLE movie_actor (
    movie_id BIGINT NOT NULL,
    person_id BIGINT NOT NULL,
    display_order INT NOT NULL,
    PRIMARY KEY (movie_id, person_id),
    CONSTRAINT fk_movie_actor_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT fk_movie_actor_person FOREIGN KEY (person_id) REFERENCES person(id)
);

CREATE TABLE movie_company (
    movie_id BIGINT NOT NULL,
    company_id BIGINT NOT NULL,
    company_role VARCHAR(100),
    display_order INT NOT NULL,
    PRIMARY KEY (movie_id, company_id, display_order),
    CONSTRAINT fk_movie_company_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT fk_movie_company_company FOREIGN KEY (company_id) REFERENCES company(id)
);

CREATE TABLE movie_provider (
    movie_id BIGINT NOT NULL,
    provider_id BIGINT NOT NULL,
    provider_type VARCHAR(20) NOT NULL,
    region_code VARCHAR(10) NOT NULL,
    display_order INT NOT NULL,
    PRIMARY KEY (movie_id, provider_id, provider_type, region_code),
    CONSTRAINT fk_movie_provider_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT fk_movie_provider_provider FOREIGN KEY (provider_id) REFERENCES provider(id)
);

CREATE TABLE movie_keyword (
    movie_id BIGINT NOT NULL,
    keyword_id BIGINT NOT NULL,
    display_order INT NOT NULL,
    PRIMARY KEY (movie_id, keyword_id),
    CONSTRAINT fk_movie_keyword_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT fk_movie_keyword_keyword FOREIGN KEY (keyword_id) REFERENCES keyword(id)
);

CREATE TABLE movie_tag (
    movie_id BIGINT NOT NULL,
    tag_id BIGINT NOT NULL,
    created_at TIMESTAMP NOT NULL,
    PRIMARY KEY (movie_id, tag_id),
    CONSTRAINT fk_movie_tag_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT fk_movie_tag_tag FOREIGN KEY (tag_id) REFERENCES tag(id)
);

CREATE TABLE batch_job_run (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    job_name VARCHAR(100) NOT NULL,
    status VARCHAR(20) NOT NULL,
    request_payload CLOB,
    result_payload CLOB,
    error_message CLOB,
    started_at TIMESTAMP NOT NULL,
    finished_at TIMESTAMP
);

CREATE TABLE kobis_movie_raw (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    kobis_movie_cd VARCHAR(20) NOT NULL UNIQUE,
    payload CLOB NOT NULL,
    imported_at TIMESTAMP NOT NULL
);

CREATE TABLE tmdb_movie_raw (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tmdb_movie_id BIGINT NOT NULL UNIQUE,
    payload CLOB NOT NULL,
    imported_at TIMESTAMP NOT NULL
);

CREATE TABLE tmdb_trending_chart (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    rank_no INT NOT NULL UNIQUE,
    tmdb_movie_id BIGINT NOT NULL UNIQUE,
    movie_id BIGINT NOT NULL UNIQUE,
    fetched_at TIMESTAMP NOT NULL,
    CONSTRAINT fk_tmdb_trending_chart_movie FOREIGN KEY (movie_id) REFERENCES movie(id)
);

CREATE TABLE movie_audit (
    movie_id BIGINT NOT NULL,
    audit_id BIGINT NOT NULL,
    watch_grade_id BIGINT,
    display_order INT NOT NULL,
    PRIMARY KEY (movie_id, audit_id, display_order),
    CONSTRAINT fk_movie_audit_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT fk_movie_audit_audit FOREIGN KEY (audit_id) REFERENCES audit(id),
    CONSTRAINT fk_movie_audit_watch_grade FOREIGN KEY (watch_grade_id) REFERENCES watch_grade(id)
);

CREATE TABLE user_movie_like (
    user_id BIGINT NOT NULL,
    movie_id BIGINT NOT NULL,
    liked BOOLEAN NOT NULL DEFAULT TRUE,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, movie_id),
    CONSTRAINT fk_user_movie_like_user FOREIGN KEY (user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_user_movie_like_movie FOREIGN KEY (movie_id) REFERENCES movie(id)
);

CREATE TABLE user_movie_store (
    user_id BIGINT NOT NULL,
    movie_id BIGINT NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, movie_id),
    CONSTRAINT fk_user_movie_store_user FOREIGN KEY (user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_user_movie_store_movie FOREIGN KEY (movie_id) REFERENCES movie(id)
);

CREATE TABLE user_movie_watched (
    user_id BIGINT NOT NULL,
    movie_id BIGINT NOT NULL,
    status VARCHAR(20) NOT NULL DEFAULT 'WATCHED',
    rating INT,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, movie_id),
    CONSTRAINT fk_user_movie_watched_user FOREIGN KEY (user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_user_movie_watched_movie FOREIGN KEY (movie_id) REFERENCES movie(id),
    CONSTRAINT ck_user_movie_watched_status CHECK (status IN ('WATCHING', 'WATCHED')),
    CONSTRAINT ck_user_movie_watched_rating CHECK (rating IS NULL OR rating BETWEEN 1 AND 5)
);

CREATE TABLE user_movie_collection (
    user_id BIGINT NOT NULL,
    movie_id BIGINT NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, movie_id),
    CONSTRAINT fk_user_movie_collection_user FOREIGN KEY (user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_user_movie_collection_movie FOREIGN KEY (movie_id) REFERENCES movie(id)
);

CREATE TABLE user_follow (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    follower_user_id BIGINT NOT NULL,
    following_user_id BIGINT NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_user_follow_follower FOREIGN KEY (follower_user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_user_follow_following FOREIGN KEY (following_user_id) REFERENCES "USER"(id),
    CONSTRAINT uk_user_follow_pair UNIQUE (follower_user_id, following_user_id),
    CONSTRAINT ck_user_follow_self CHECK (follower_user_id <> following_user_id)
);

CREATE INDEX idx_user_follow_follower ON user_follow (follower_user_id, created_at);
CREATE INDEX idx_user_follow_following ON user_follow (following_user_id, created_at);

CREATE TABLE user_movie_life (
    user_id BIGINT NOT NULL,
    movie_id BIGINT NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, movie_id),
    CONSTRAINT fk_user_movie_life_user FOREIGN KEY (user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_user_movie_life_movie FOREIGN KEY (movie_id) REFERENCES movie(id)
);

CREATE TABLE user_recommendation_refresh_state (
    user_id BIGINT NOT NULL PRIMARY KEY,
    pending_event_count INT NOT NULL DEFAULT 0,
    last_refreshed_at TIMESTAMP,
    last_event_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    last_refresh_event_count INT NOT NULL DEFAULT 0,
    algorithm_version VARCHAR(50) NOT NULL,
    dirty BOOLEAN NOT NULL DEFAULT TRUE,
    CONSTRAINT fk_user_recommendation_refresh_state_user FOREIGN KEY (user_id) REFERENCES "USER"(id)
);

CREATE INDEX idx_recommendation_refresh_state_dirty
ON user_recommendation_refresh_state (dirty, last_event_at);

CREATE TABLE user_preference_profile (
    user_id BIGINT NOT NULL,
    feature_type VARCHAR(20) NOT NULL,
    feature_name VARCHAR(255) NOT NULL,
    score DOUBLE PRECISION NOT NULL,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, feature_type, feature_name),
    CONSTRAINT fk_user_preference_profile_user FOREIGN KEY (user_id) REFERENCES "USER"(id)
);

CREATE TABLE user_recommendation_result (
    user_id BIGINT NOT NULL,
    movie_id BIGINT NOT NULL,
    rank_no INT NOT NULL,
    final_score DOUBLE PRECISION NOT NULL,
    tag_score DOUBLE PRECISION NOT NULL,
    genre_score DOUBLE PRECISION NOT NULL,
    people_score DOUBLE PRECISION NOT NULL,
    keyword_score DOUBLE PRECISION NOT NULL,
    provider_score DOUBLE PRECISION NOT NULL,
    penalty_score DOUBLE PRECISION NOT NULL,
    reason_summary VARCHAR(500),
    algorithm_version VARCHAR(50) NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, movie_id),
    CONSTRAINT uk_user_recommendation_result_rank UNIQUE (user_id, rank_no),
    CONSTRAINT fk_user_recommendation_result_user FOREIGN KEY (user_id) REFERENCES "USER"(id),
    CONSTRAINT fk_user_recommendation_result_movie FOREIGN KEY (movie_id) REFERENCES movie(id)
);

CREATE TABLE movie_co_occurrence (
    movie_id_a BIGINT NOT NULL,
    movie_id_b BIGINT NOT NULL,
    co_count   INT    NOT NULL DEFAULT 1,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (movie_id_a, movie_id_b),
    CONSTRAINT fk_movie_co_occurrence_a FOREIGN KEY (movie_id_a) REFERENCES movie(id),
    CONSTRAINT fk_movie_co_occurrence_b FOREIGN KEY (movie_id_b) REFERENCES movie(id)
);

CREATE INDEX idx_movie_co_occurrence_a ON movie_co_occurrence (movie_id_a, co_count DESC);
CREATE INDEX idx_movie_co_occurrence_b ON movie_co_occurrence (movie_id_b, co_count DESC);

CREATE TABLE IF NOT EXISTS social_post (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id BIGINT NOT NULL,
    movie_id BIGINT NOT NULL,
    content TEXT,
    is_deleted BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE IF NOT EXISTS social_post_image (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    post_id BIGINT NOT NULL,
    image_url VARCHAR(500) NOT NULL,
    display_order INT NOT NULL DEFAULT 0,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE TABLE IF NOT EXISTS movie_throw (
    id               BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    sender_user_id   BIGINT NOT NULL,
    receiver_user_id BIGINT NOT NULL,
    movie_id         BIGINT,
    movie_cd         VARCHAR(20),
    created_at       TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_throw_sender   FOREIGN KEY (sender_user_id)   REFERENCES "USER"(id),
    CONSTRAINT fk_throw_receiver FOREIGN KEY (receiver_user_id) REFERENCES "USER"(id)
);
CREATE INDEX IF NOT EXISTS idx_throw_sender   ON movie_throw (sender_user_id,   created_at DESC);
CREATE INDEX IF NOT EXISTS idx_throw_receiver ON movie_throw (receiver_user_id, created_at DESC);

CREATE TABLE user_badge_earned (
    user_id    BIGINT      NOT NULL,
    badge_code VARCHAR(50) NOT NULL,
    earned_at  TIMESTAMP   NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, badge_code),
    CONSTRAINT fk_user_badge_earned_user FOREIGN KEY (user_id) REFERENCES "USER"(id)
);

CREATE TABLE user_badge_selected (
    user_id    BIGINT      NOT NULL PRIMARY KEY,
    badge_code VARCHAR(50),
    updated_at TIMESTAMP   NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_user_badge_selected_user FOREIGN KEY (user_id) REFERENCES "USER"(id)
);
