...
public class RecommendationFeaturePolicy {
    /*
     * 추천 정책 상수들을 한 곳에 모아둔 클래스다.
     * "왜 이 영화가 추천됐는지"를 설명 가능하게 유지하려고,
     * 행동 가중치/feature limit/정규화 계수/블록 생성 기준을 코드에 명시적으로 분리했다.
     */
    ...
    public double watchedSignalWeight(Integer rating) {
        // watched는 like보다 약하지만 store보다 강한 신호로 두고, rating으로 세기를 보정한다.
    ...
    public double featureSpecificWeight(String featureType, String featureName) {
        // broad genre는 같은 점수를 그대로 쓰면 추천이 넓게 퍼지므로 장르별 감쇠를 둔다.
