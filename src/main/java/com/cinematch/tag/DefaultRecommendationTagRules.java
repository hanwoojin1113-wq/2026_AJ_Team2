...
public class DefaultRecommendationTagRules {
    /*
     * 태그 규칙 정의 모음이다.
     * 각 태그는 장르/키워드/런타임 조건을 조합한 "설명 가능한 규칙"으로 만든다.
     *
     * 예:
     * - tense: thriller, mystery, suspenseful 같은 신호가 강할수록 점수 상승
     * - healing: 가족/드라마 + comforting/healing 키워드가 있을 때 점수 상승
     * - with_family: 공포/고어 같은 hard exclude가 있으면 아예 탈락
     */
    ...
    private List<TagRule> buildRules() {
        // 여기서 반환하는 순서는 score 계산 순서가 아니라 "지원하는 태그 목록" 자체를 의미한다.
        return List.of(
                funny(),
                tense(),
                dark(),
    ...
    private TagRule funny() {
        // 코미디/가족/가벼운 분위기를 기반으로 "funny" 태그를 계산한다.
    ...
    private TagRule dark() {
        // 공포/범죄/비극/우울 키워드를 묶어 어두운 분위기를 나타낸다.
    ...
    private TagRule emotional() {
        // 감정 소모가 큰 영화, 눈물/상실/정서적 성장 계열 신호를 반영한다.
    ...
    private TagRule romantic() {
        // 로맨스 장르를 강하게 요구해서 연애 분위기 태그가 엉뚱한 영화에 붙지 않게 한다.
    ...
    private TagRule healing() {
        // "healing"은 가족/일상/위로 계열 신호를 강조하고 공포 장르는 강하게 배제한다.
    ...
    private TagRule trueStory() {
        // 실화 기반 여부는 genre보다 keyword 신호를 더 강하게 본다.
