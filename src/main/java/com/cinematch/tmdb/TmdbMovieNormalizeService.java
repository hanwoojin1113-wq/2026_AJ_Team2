...
public class TmdbMovieNormalizeService {
    ...
    private NormalizeResult normalizeRawMovieRows(List<RawMovieRow> rawMovies) {
        ...
        for (RawMovieRow rawMovie : rawMovies) {
            ...
            normalizeVideos(movieId, root.path("videos").path("results"));
        }
    ...
    private void initializeVideoTable() {
        jdbcTemplate.execute("""
                CREATE TABLE IF NOT EXISTS movie_video (
                    ...
                    source VARCHAR(20),
                    display_order INT NOT NULL,
                    CONSTRAINT fk_movie_video_movie FOREIGN KEY (movie_id) REFERENCES movie(id)
                )
                """);
        try {
    ...
    private void normalizeVideos(Long movieId, JsonNode videosNode) {
        if (!videosNode.isArray()) {
            return;
        }

        // YouTube 영상만 추출, Trailer → Teaser → 나머지 순으로 정렬
        List<JsonNode> youtubeVideos = new ArrayList<>();
        for (JsonNode v : videosNode) {
            if ("YouTube".equals(textValue(v, "site")) && textValue(v, "key") != null) {
                youtubeVideos.add(v);
            }
        }
        ...
        youtubeVideos.sort((a, b) -> {
            int scoreA = videoTypeScore(textValue(a, "type"), booleanValue(a, "official"));
            int scoreB = videoTypeScore(textValue(b, "type"), booleanValue(b, "official"));
            return Integer.compare(scoreA, scoreB);
        });

        jdbcTemplate.update("DELETE FROM movie_video WHERE movie_id = ?", movieId);

        int order = 1;
        for (JsonNode v : youtubeVideos) {
            jdbcTemplate.update("""
                    ...
                    textValue(v, "name"),
                    textValue(v, "type"),
                    booleanValue(v, "official"),
                    order++
            );
        }
    }

    private int videoTypeScore(String type, Boolean official) {
        int typeScore = switch (type == null ? "" : type) {
            case "Trailer" -> 0;
            case "Teaser" -> 1;
            case "Clip" -> 2;
            default -> 3;
        };
        int officialBonus = Boolean.TRUE.equals(official) ? 0 : 10;
        return typeScore + officialBonus;
    }
